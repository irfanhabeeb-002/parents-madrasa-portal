...
export const PWATestSuite: React.FC = () => {
  ...
  const updateTestState = useCallback(
    (testName: string, state: Partial<TestStatus>) => {
      setTestStates(prev => ({
        ...prev,
      ...
      try {
        ...
        const passed =
          result.isValid !== undefined
            ? result.isValid
            : result.isInstallable !== undefined
              ? result.isInstallable
              : result.isRegistered !== undefined
                ? result.isRegistered
    ...
    try {
      ...
      Object.entries(allResults).forEach(([key, result]) => {
        if (key !== 'timestamp') {
          ...
          const passed =
            (result as any).isValid !== undefined
              ? (result as any).isValid
              : (result as any).isInstallable !== undefined
                ? (result as any).isInstallable
                : (result as any).isRegistered !== undefined
                  ? (result as any).isRegistered
                  : (result as any).passed !== undefined
  ...
  const calculateOverallScore = () => {
    const completedTests = Object.values(testStates).filter(
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <div className="text-center">
        ...
        <p className="text-lg text-gray-600 max-w-3xl mx-auto mb-6">
          Comprehensive testing and validation of Progressive Web App
          functionality. Test offline capabilities, installation flow,
        ...
        {offlineSimulation && (
          <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded">
            <p className="text-sm text-yellow-800">
              ⚠️ Offline simulation is active. Network requests will be blocked
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <ul className="text-sm text-gray-700 space-y-1">
              <li>
                • Test installation on different browsers (Chrome, Edge,
                Firefox)
              </li>
