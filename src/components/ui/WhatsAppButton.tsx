...
export interface WhatsAppButtonProps {
  ...
  context?: 'general' | 'class_help' | 'technical_support' | 'homework_help' | 'exam_query';
...
const MALAYALAM_TEMPLATES = {
  ...
  class_help: "അസ്സലാമു അലൈകും, ഇന്നത്തെ ക്ലാസിനെക്കുറിച്ച് എനിക്ക് ഒരു ചോദ്യമുണ്ട്.",
  technical_support: "അസ്സലാമു അലൈകും, പോർട്ടലിൽ എനിക്ക് സാങ്കേതിക പ്രശ്നങ്ങളുണ്ട്.",
  homework_help: "അസ്സലാമു അലൈകും, ഗൃഹപാഠത്തിൽ എനിക്ക് സഹായം വേണം.",
  exam_query: "അസ്സലാമു അലൈകും, വരാനിരിക്കുന്ന പരീക്ഷയെക്കുറിച്ച് എനിക്ക് ഒരു ചോദ്യമുണ്ട്."
} as const;
...
export const WhatsAppButton: React.FC<WhatsAppButtonProps> = ({
  ...
  const accessibleLabel = ariaLabel || `Ask teacher on WhatsApp - ${getWhatsAppMessage()}`;
      ...
      {showLabel && (isHovered || isFocused) && (
        ...
        >
          <div className="flex flex-col items-center">
            ...
            {malayalamLabel && (
              <span className="text-xs opacity-80 mt-1 whitespace-nowrap" lang="ml">
                {malayalamLabel}
              </span>
            )}
          </div>
