...
describe('Data Operations Integration Tests', () => {
  ...
  describe('Announcements Data Flow', () => {
    ...
    it('should filter announcements by target audience', async () => {
      const studentsResult = await DashboardService.getAnnouncements('students')
  ...
  describe('Notifications Data Flow', () => {
    ...
    it('should filter notifications by read status', async () => {
      const userId = '1'
      const allNotifications = await DashboardService.getUserNotifications(userId, true)
      const unreadOnly = await DashboardService.getUserNotifications(userId, false)
  ...
  describe('Class Data Flow', () => {
    it('should fetch upcoming classes correctly', async () => {
      ...
      result.data.forEach(classSession => {
        ...
        expect(new Date(classSession.scheduledAt).getTime()).toBeGreaterThan(now.getTime())
        ...
        expect(['scheduled', 'live', 'ended', 'cancelled']).toContain(classSession.status)
      })
  ...
  describe('Data Persistence Integration', () => {
    ...
    it('should handle storage errors gracefully', async () => {
      ...
      vi.mocked(StorageService.setArray).mockRejectedValue(new Error('Storage error'))
  ...
  describe('Real-time Data Subscriptions', () => {
    ...
    it('should set up user notification subscriptions', () => {
      ...
      const unsubscribe = DashboardService.subscribeToUserNotifications(userId, callback)
