...
export const useZoom = (options: UseZoomOptions = {}): UseZoomReturn => {
  ...
  const [meetingStatus, setMeetingStatus] = useState<ZoomMeetingSDKStatus | null>(null);
  const [error, setError] = useState<ZoomError | null>(null);
  const [attendanceRecord, setAttendanceRecord] = useState<ZoomAttendanceRecord | null>(null);
   ...
   */
  const trackAttendanceAction = useCallback(async (action: 'join' | 'leave') => {
   ...
   * Join a Zoom meeting
   */
  const joinMeeting = useCallback(async (config: Omit<ZoomMeetingConfig, 'role'>): Promise<boolean> => {
    try {
      setError(null);
      setIsJoining(true);
   ...
   */
  const generateMeetingSignature = useCallback((meetingNumber: string): string => {
    try {
      return zoomService.generateSignature(meetingNumber, STUDENT_MEETING_SETTINGS.role);
    } catch (err) {
