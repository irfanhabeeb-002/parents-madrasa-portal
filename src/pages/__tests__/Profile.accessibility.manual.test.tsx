...
describe('Profile Accessibility Manual Verification', () => {
  ...
  test('confirmation dialog has proper ARIA attributes', async () => {
    ...
    expect(screen.getByText('Confirm Logout')).toHaveAttribute(
      'id',
      'logout-dialog-title'
    );
    expect(screen.getByText(/Are you sure you want to logout/)).toHaveAttribute(
      'id',
      'logout-dialog-description'
    );
  });
  ...
  test('confirmation dialog buttons have proper accessibility', async () => {
    ...
    const confirmSrText = document.querySelector(
      '#confirm-logout-button-sr-text'
    );
    const cancelSrText = document.querySelector(
      '#cancel-logout-button-sr-text'
    );

    expect(confirmSrText).toHaveTextContent(
      'Click to confirm logout and sign out of your account'
    );
    expect(cancelSrText).toHaveTextContent(
      'Click to cancel logout and remain logged in'
    );
  });
  ...
  test('error recovery buttons have proper accessibility when logout fails', async () => {
    ...
    expect(forceSrText).toHaveTextContent(
      'Click to force logout and clear all stored session data. This will immediately log you out.'
    );
  });
  ...
  test('logout description has proper ID for aria-describedby', () => {
    ...
    expect(logoutButton.getAttribute('aria-describedby')).toContain(
      'logout-description'
    );
  });
