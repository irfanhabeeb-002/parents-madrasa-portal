...
describe('Profile Visual Regression Tests', () => {
  ...
  describe('Layout Consistency Across Breakpoints', () => {
    const breakpoints = [
      { width: 320, name: 'Small Mobile', expectedClasses: ['px-2', 'text-xl', 'text-sm'] },
      { width: 375, name: 'Standard Mobile', expectedClasses: ['px-2', 'text-xl', 'text-sm'] },
      { width: 414, name: 'Large Mobile', expectedClasses: ['px-2', 'text-xl', 'text-sm'] },
      { width: 640, name: 'Small Tablet', expectedClasses: ['sm:px-4', 'sm:text-2xl', 'sm:text-base'] },
      { width: 768, name: 'Tablet', expectedClasses: ['sm:px-4', 'sm:text-2xl', 'sm:text-base'] },
      { width: 1024, name: 'Desktop', expectedClasses: ['md:px-0', 'lg:text-3xl', 'lg:text-lg'] }
    ]
  ...
  describe('Visual Hierarchy Consistency', () => {
    it('should maintain visual hierarchy across screen sizes', () => {
      ...
      sizes.forEach(width => {
        ...
        const mainTitle = screen.getAllByRole('heading', { name: /profile/i })[0]
        const cardTitles = screen.getAllByRole('heading', { level: 3 })
        
        const mainTitleStyles = window.getComputedStyle(mainTitle)
        const cardTitleStyles = cardTitles.map(title => window.getComputedStyle(title))
        
        const mainTitleSize = parseInt(mainTitleStyles.fontSize)
        const cardTitleSizes = cardTitleStyles.map(styles => parseInt(styles.fontSize))
  ...
  describe('Animation and Transition Consistency', () => {
    ...
    it('should maintain consistent spacing transitions', () => {
      ...
      expect(tabletMetrics.containerWidth).toBeGreaterThanOrEqual(mobileMetrics.containerWidth)
