...
export const ExamsAttendance: React.FC = () => {
  ...
  const [currentAttempt, setCurrentAttempt] = useState<ExamAttempt | null>(
    null
  );
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [exercisesResponse, resultsResponse, statsResponse] =
        await Promise.all([
  ...
  const handleStartExam = async (exercise: Exercise) => {
    ...
    try {
      setLoading(true);
      const attemptResponse = await ExerciseService.startExamAttempt(
        exercise.id,
        user.uid
      );
  ...
  const handleSubmitExam = async () => {
    ...
    try {
      setLoading(true);
      const resultResponse = await ExerciseService.submitExam(
        currentAttempt.id
      );
  ...
  const canTakeExam = (exercise: Exercise): boolean => {
    const userAttempts = userResults.filter(
              ...
              return (
                ...
                >
                  ...
                  <div className="text-right">
                    <p
                      className={`font-bold ${result.score >= (exercise?.passingScore || 70) ? 'text-success-600' : 'text-error-600'}`}
                    >
                      {result.score}%
                    </p>
                    <p className="text-xs text-gray-500">
                      {result.score >= (exercise?.passingScore || 70)
                        ? 'Passed'
                        : 'Failed'}
                    </p>
                  </div>
  ...
  const renderExamsList = () => (
    <div className="space-y-4">
      ...
      {exercises.map(exercise => {
        ...
        const attemptsUsed = userResults.filter(
          ...
          <div key={exercise.id} className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                {userResult && (
                  <div
                    className={`p-3 rounded-lg ${userResult.score >= exercise.passingScore ? 'bg-success-50 border border-success-200' : 'bg-error-50 border border-error-200'}`}
                  >
                    <p className="font-medium">
                      Last Score: {userResult.score}%
                      <span className="ml-2">
                        {userResult.score >= exercise.passingScore
                          ? '✅ Passed'
                          : '❌ Failed'}
                      </span>
                    </p>
  ...
  const renderExamTaking = () => {
    ...
    const isLastQuestion =
      examFormData.currentQuestionIndex === currentExam.questions.length - 1;
      ...
      <div className="max-w-4xl mx-auto">
                ...
                >
                  ...
                  <div>
                    ...
                    {option.textMalayalam && (
                      <span className="text-gray-600 text-sm ml-2" lang="ml">
                        ({option.textMalayalam})
                      </span>
                    )}
                  </div>
        ...
        <div className="flex justify-between items-center">
          ...
          <span className="text-sm text-gray-600">
            Question {examFormData.currentQuestionIndex + 1} of{' '}
  ...
  const renderAttendance = () => {
    const attendanceData: Record<
      string,
      { status: any; duration?: number; notes?: string }
    > = {};
