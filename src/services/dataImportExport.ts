...
export class DataImportExportService {
  ...
  static async downloadDataAsFile(dataType: DataType = 'all', filename?: string): Promise<ApiResponse<boolean>> {
    try {
  ...
  static async importData(
    ...
    try {
      const { replaceExisting = false, validateData = true, skipDuplicates = true } = options;
      ...
      if (validateData && !this.validateImportData(importData)) {
        return {
          data: { success: false, imported: 0, skipped: 0, errors: ['Invalid data format'], collections: [] },
          success: false,
      ...
      return {
        data: { success: false, imported: 0, skipped: 0, errors: [error instanceof Error ? error.message : 'Import failed'], collections: [] },
        success: false,
  ...
  static async importFromFile(file: File, options?: {
    ...
    try {
      if (!file.type.includes('json')) {
        return {
          data: { success: false, imported: 0, skipped: 0, errors: ['File must be JSON format'], collections: [] },
          success: false,
      ...
      return {
        data: { success: false, imported: 0, skipped: 0, errors: [error instanceof Error ? error.message : 'File read failed'], collections: [] },
        success: false,
  ...
  private static async importCollection<T extends { id: string }>(
    ...
    try {
      ...
      result.errors.push(`Failed to import ${collectionName}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  ...
  private static validateImportData(data: any): data is ExportData {
    ...
    if (typeof metadata.totalRecords !== 'number' || !Array.isArray(metadata.collections)) {
      return false;
    }
    ...
    const collections = ['recordings', 'notes', 'exercises', 'attendance', 'users'];
  ...
  // Create backup
  static async createBackup(includeUserData: boolean = true): Promise<ApiResponse<string>> {
    try {
  ...
  // Restore from backup
  static async restoreFromBackup(backupKey: string): Promise<ApiResponse<ImportResult>> {
    try {
      const backupData = StorageService.get<ExportData>(backupKey);
      
      if (!backupData) {
        return {
          data: { success: false, imported: 0, skipped: 0, errors: ['Backup not found'], collections: [] },
          success: false,
      ...
      return this.importData(backupData, { replaceExisting: true, validateData: true });
    } catch (error) {
      return {
        data: { success: false, imported: 0, skipped: 0, errors: [error instanceof Error ? error.message : 'Restore failed'], collections: [] },
        success: false,
